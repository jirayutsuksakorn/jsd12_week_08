...
export default function SpaceGame({ onBack }) {
    ...
    // 2. ระบบ Multiplayer: ติดตามเพื่อนคนอื่นๆ
    useEffect(() => {
    ...
    // 3. ส่งพิกัดตำแหน่งของเราไปให้เพื่อนเห็น
    useEffect(() => {
    ...
    const shoot = () => {
        ...
        if (powerLevel === 1) {
            setBullets(prev => [...prev, { id: Date.now() + Math.random(), x: playerX, y: 85, angle: 0 }]);
        } else {
            const now = Date.now();
            setBullets(prev => [
                { id: now + 1, x: playerX, y: 85, angle: -1.5 },
                { id: now + 2, x: playerX, y: 85, angle: 0 },
                { id: now + 3, x: playerX, y: 85, angle: 1.5 },
                ...prev
            ]);
        }
    };

    // 4. Main Game Loop (Movement & Collision)
    useEffect(() => {
        if (gameState !== "PLAY") return;
        ...
        const interval = setInterval(() => {
            ...
            setEnemies(prev => {
                ...
                if (Math.random() < 0.05) next.push({ id: Date.now() + Math.random(), x: Math.random() * 90 + 5, y: -10 });
            ...
            setItems(prev => {
                ...
                if (Math.random() < 0.01) next.push({ id: Date.now() + Math.random(), x: Math.random() * 90 + 5, y: -10 });
